...
```{r}
# Seleccionar todas las columnas desde r_arch hasta country
cran_range <- cran %>%
...
```{r}
# Seleccionar todas las columnas excepto las que están entre date y size
cran_excluded <- cran %>%
  select(-(date:size))
...
```{r}
# Filtrar registros donde el paquete sea "plotly" y la versión de R sea "3.5.1"
cran_plotly <- cran %>%
...
```{r}
# Filtrar registros donde el país sea "ES" y la versión de R sea mayor que "3.4.0"
cran_es <- cran %>%
...
```{r}
# Filtrar registros donde el país sea "ES" o "PT"
cran_es_pt <- cran %>%
