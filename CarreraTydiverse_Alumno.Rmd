...
pack_sum <- by_package %>%
  summarise(
    count = n(),  # Número de elementos en cada grupo
    unique = n_distinct(ip_id),  # Número de valores únicos de ip_id
    countries = n_distinct(country),  # Número de valores únicos de country
    avg_bytes = mean(size, na.rm = TRUE),  # Media de size
    med_bytes = median(size, na.rm = TRUE)) #Mediana de size
...
# Calcular el umbral del percentil 99 para count
umbral <- quantile(pack_sum$count, probs = 0.99)

# Filtrar los paquetes que superan el umbral y almacenarlos en top_counts
top_counts <- cran %>%
  group_by(package) %>%  # Agrupar por paquete
  summarise(count = n()) %>%  # Calcular el número de descargas por paquete
  filter(count > umbral)  # Filtrar los paquetes que superan el umbral
...
# Calcular el 1% de los paquetes más descargados y ordenar en orden descendente
top_counts_sorted <- cran %>%
  group_by(package) %>%  # Agrupar por paquete
  summarise(count = n()) %>%  # Calcular el número de descargas por paquete
  filter(count > quantile(count, probs = 0.99)) %>%  # Filtrar los paquetes que superan el umbral
  arrange(desc(count))  # Ordenar en orden descendente según count
